...
Page({
  ...
  FindPerson:function(){
    ...
    var updatelist = wx.getStorageSync('person_list')
    for (let i = 0; i < updatelist.length; i++) {
      if (updatelist[i].id == this.data.Person.id) {
        updatelist[i] = this.data.Person;
      }
    }
    wx.setStorageSync('person_list',updatelist)
  ...
  save: function () {
    ...
    wx.setStorageSync('personly_list', this.data.lylist)
  },
  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function (options) {
    ...
    var pylist = wx.getStorageSync('personly_list')
